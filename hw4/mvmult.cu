...
void vvmult(double* x, double* y, double* z, double* sum, const long N)
{
  ...
  cudaDeviceSynchronize();
  total_gpu_time += omp_get_wtime() - tt;
...
int main() {
  ...
  for (long i = 0; i < N; i++) 
  {
      ...
      double tt = omp_get_wtime();
      reduction(sum_ref + i, z, N);
      total_cpu_time += omp_get_wtime() - tt;
  }


  printf("CPU Bandwidth = %f GB/s\n", 1*N*N*sizeof(double) / total_cpu_time/1e9);
  // double sum_ref, sum;
  double *sum;
  ...
  #pragma omp parallel for schedule(static)
  for (long i = 0; i < N; i++)
    vvmult(x + i * N, y, z, sum + i, N);

  printf("GPU Bandwidth = %f GB/s\n", 1*N*N*sizeof(double) / total_gpu_time /1e9);

  double max_error = 0.0;
  for (long i = 0; i < N; i++)
  {
    if (abs(sum[i] - sum_ref[i]) > max_error)
      max_error = abs(sum[i] - sum_ref[i]);
  }
  printf("Error = %f\n", max_error);
