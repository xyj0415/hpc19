...
#if defined(__AVX__)
  __m256d x1, x2, x3, x5, x7, x9, x11, x13, x15;
  x1  = _mm256_load_pd(x);
...
// }
  ...
  //   // The definition of intrinsic functions can be found at:
  //   // https://software.intel.com/sites/landingpage/IntrinsicsGuide/#
  //   __m512d x1, x2, x3, x5, x7, x9, x11, x13, x15;
  //   x1 = _mm512_load_pd(x);
  //   x2 = _mm512_mul_pd(x1, x1);
  //   x3 = _mm512_mul_pd(x1, x2);
  //   x5 = _mm512_mul_pd(x3, x2);
  //   x7 = _mm512_mul_pd(x5, x2);
  //   x9 = _mm512_mul_pd(x7, x2);
  //   x11 = _mm512_mul_pd(x9, x2);
  //   //   x13  = _mm512_mul_pd(x11, x2);
  //   //   x15  = _mm512_mul_pd(x13, x2);

  //   __m512d s = x1;
  //   s = _mm512_add_pd(s, _mm512_mul_pd(x3, _mm512_set1_pd(c3)));
  //   s = _mm512_add_pd(s, _mm512_mul_pd(x5, _mm512_set1_pd(c5)));
  //   s = _mm512_add_pd(s, _mm512_mul_pd(x7, _mm512_set1_pd(c7)));
  //   s = _mm512_add_pd(s, _mm512_mul_pd(x9, _mm512_set1_pd(c9)));
  //   s = _mm512_add_pd(s, _mm512_mul_pd(x11, _mm512_set1_pd(c11)));
  //   //   s = _mm512_add_pd(s, _mm512_mul_pd(x13 , _mm512_set1_pd(c13)));
  //   //   s = _mm512_add_pd(s, _mm512_mul_pd(x15 , _mm512_set1_pd(c15)));
  //   _mm512_store_pd(sinx, s);
  // }
  ...
  int main()
  {
    ...
    //   for (long i = 0; i < N; i += 4)
    //   {
    //     sin4_intrin2(sinx_intrin + i, x + i);
    //   }
    // }
    // printf("Intrin time:    %6.4f      Error: %e\n", tt.toc(), err(sinx_ref, sinx_intrin, N));
